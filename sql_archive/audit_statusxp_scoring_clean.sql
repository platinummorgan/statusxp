-- STATUSXP AUDIT FOR USER: 84b60ad6-cb2c-484f-8953-bf814551fd7a

-- Platform totals (calculated)
SELECT 
  CASE 
...
-- Per-game delta: calculated vs stored current_score
WITH calc AS (
...
-- Platform totals delta: calculated vs stored
WITH calc AS (
...
-- Rarity multiplier distribution for earned achievements
WITH user_ach AS (
  SELECT a.rarity_multiplier
  FROM achievements a
...
-- Base StatusXP distribution (should be 5/7/9/12/15; 0 for excluded platinums)
WITH user_ach AS (
  SELECT a.base_status_xp
  FROM achievements a
  JOIN user_achievements ua ON ua.platform_id = a.platform_id
    AND ua.platform_game_id = a.platform_game_id
    AND ua.platform_achievement_id = a.platform_achievement_id
...
-- Sanity: any achievements over 15?
