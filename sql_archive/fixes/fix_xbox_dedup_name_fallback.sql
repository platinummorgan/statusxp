-- Three-tier deduplication: xbox_title_id > game name > game_title_id
...
BEGIN
  ...
  FROM profiles p
  LEFT JOIN (
    -- Deduplicate: Use xbox_title_id if available, otherwise deduplicate by game name
    SELECT 
      user_id,
    ...
    FROM (
      SELECT 
        ug.user_id,
        -- Use xbox_title_id if available, otherwise use 'NAME:' prefix + game name
