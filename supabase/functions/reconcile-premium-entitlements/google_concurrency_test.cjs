...
(async () => {
  try {
    sql(`INSERT INTO auth.users VALUES('${user}');
      SELECT public.fulfill_google_purchase_with_token('${user}','google_play','race_play_1','statusxp_premium_monthly','subscription',
        'SUBSCRIPTION_STATE_ACTIVE',now(),now()+interval '30 days',false,
        jsonb_build_object('subscriptionKey','${key}','accountBound',true,'verifiedAt',now()),${envelope});`);
    const negative =
      `SELECT public.apply_google_notification_with_token('9001',repeat('a',64),now()+interval '10 seconds',
      jsonb_build_object('subscriptionKey','${key}','state','SUBSCRIPTION_STATE_EXPIRED','expiresAt',now()),'${user}',${envelope});`;
    const mobile =
      `SELECT public.fulfill_google_purchase_with_token('${user}','google_play','race_play_2','statusxp_premium_monthly','subscription',
    ...
    assert.equal(
      sql(
        `SELECT count(*) FROM public.google_play_tokens WHERE subscription_key='${key}'`,
      ),
      "1",
    );
    assert.equal(
      sql(
        `SELECT count(*) FROM public.entitlement_reconciliation_jobs WHERE provider='google' AND subject='${key}'`,
      ),
      "1",
    );
    console.log(
      "Concurrent duplicate expiry notifications and an older mobile verification leave one receipt, one encrypted token/job, and no restored coverage.",
    );
  } finally {
    sql(
