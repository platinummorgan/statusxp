-- When a seasonal override is active, treat that as a hard reset:
-- use first in-period snapshot as baseline (or current if none).
-- Otherwise, keep normal behavior (last pre-period snapshot fallback).
  ...
  SELECT
    ...
    CASE
      ...
      ELSE (
        ...
        - (((extract(dow from timezone('UTC', now()))::int - 2 + 7) % 7) * interval '1 day')
      )
...
baseline_scores AS (
  SELECT
    cs.user_id,
    CASE
      WHEN b.period_start <> b.default_start
        THEN COALESCE(bip.baseline_total, cs.current_total)
      ELSE COALESCE(bb.baseline_total, bip.baseline_total, cs.current_total)
...
baseline_scores AS (
  SELECT
    ...
    CASE
      WHEN b.period_start <> b.default_start
        THEN COALESCE(bip.baseline_platinum, cs.platinum_count)
      ELSE COALESCE(bb.baseline_platinum, bip.baseline_platinum, cs.platinum_count)
    END AS baseline_platinum
  FROM current_scores cs
  CROSS JOIN bounds b
  LEFT JOIN baseline_before bb ON bb.user_id = cs.user_id
  LEFT JOIN baseline_in_period bip ON bip.user_id = cs.user_id
...
baseline_scores AS (
  SELECT
    ...
    CASE
      WHEN b.period_start <> b.default_start
        THEN COALESCE(bip.baseline_gamerscore, cs.gamerscore)
      ELSE COALESCE(bb.baseline_gamerscore, bip.baseline_gamerscore, cs.gamerscore)
    END AS baseline_gamerscore
  FROM current_scores cs
  CROSS JOIN bounds b
  LEFT JOIN baseline_before bb ON bb.user_id = cs.user_id
  LEFT JOIN baseline_in_period bip ON bip.user_id = cs.user_id
...
baseline_scores AS (
  SELECT
    ...
    CASE
      WHEN b.period_start <> b.default_start
        THEN COALESCE(bip.baseline_achievements, cs.achievement_count)
      ELSE COALESCE(bb.baseline_achievements, bip.baseline_achievements, cs.achievement_count)
    END AS baseline_achievements
  FROM current_scores cs
  CROSS JOIN bounds b
  LEFT JOIN baseline_before bb ON bb.user_id = cs.user_id
  LEFT JOIN baseline_in_period bip ON bip.user_id = cs.user_id
