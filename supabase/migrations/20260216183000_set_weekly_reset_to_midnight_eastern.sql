-- Move weekly period rollover to Tuesday 12:00 AM America/New_York.
-- This ensures "through local midnight" weekly behavior for US Eastern users.
...
BEGIN
  ...
  ELSE
    -- Tuesday-based weekly boundary at 12:00 AM America/New_York.
    v_base := (
      (
        date_trunc('day', timezone('America/New_York', p_reference))
        - (((extract(dow from timezone('America/New_York', p_reference))::int - 2 + 7) % 7) * interval '1 day')
...
-- Keep this active weekly window aligned to ET midnight for this week's rollover.
INSERT INTO public.seasonal_period_overrides (period_type, start_at, end_at, note, updated_at)
VALUES (
  'weekly',
  '2026-02-10 05:00:00+00',
  '2026-02-17 05:00:00+00',
  'One-time reset aligned to Tuesday 12:00 AM America/New_York',
  now()
)
...
-- Keep the helper in sync (uses Hall of Fame under the hood)
