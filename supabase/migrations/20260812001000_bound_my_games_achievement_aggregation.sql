-- Bound achievement aggregation to the requesting user's library.
-- The previous function grouped the entire achievements table on every My Games
-- request, which times out for large production catalogs and large libraries.
...
AS $$
WITH user_games_base AS MATERIALIZED (
  SELECT
    up.platform_id,
    up.platform_game_id,
  ...
  LEFT JOIN public.platforms p ON p.id = up.platform_id
...
achievement_totals AS (
  ...
  JOIN public.achievements a
    ON a.platform_id = ugb.platform_id
   AND a.platform_game_id = ugb.platform_game_id
...
earned_agg AS (
  SELECT
    ...
    COALESCE(SUM(CASE WHEN a.include_in_score THEN a.base_status_xp ELSE 0 END), 0)::numeric AS statusxp,
...
SELECT
  ...
  COALESCE(ea.statusxp, 0),
  COALESCE(ugb.completion_percent, 0),
  COALESCE(ea.last_earned_at, ugb.last_played_at),
  ARRAY[ugb.game_title_id]
FROM user_games_base ugb
...
ORDER BY COALESCE(ea.last_earned_at, ugb.last_played_at) DESC NULLS LAST,
         ugb.game_title;
$$;

GRANT EXECUTE ON FUNCTION public.get_user_grouped_games(uuid)
