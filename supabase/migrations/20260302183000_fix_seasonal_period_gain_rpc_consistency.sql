-- Ensure seasonal leaderboard period_gain uses earned_at windows consistently.
-- This migration removes any conflicting overloaded versions and recreates a
-- single canonical implementation for each seasonal RPC.
...
BEGIN
  FOR fn IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'get_statusxp_period_leaderboard'
  LOOP
  ...
  FOR fn IN
    ...
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public'
      AND p.proname = 'get_psn_period_leaderboard'
  LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS %s', fn);
  END LOOP;
  ...
  FOR fn IN
    ...
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public'
      AND p.proname = 'get_xbox_period_leaderboard'
  LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS %s', fn);
  END LOOP;
  ...
  FOR fn IN
    ...
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public'
      AND p.proname = 'get_steam_period_leaderboard'
  LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS %s', fn);
  END LOOP;
END;
