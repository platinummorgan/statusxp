-- ROLLBACK SCRIPT for 20260130000001_emergency_enable_rls_critical_tables.sql
-- ONLY RUN THIS IF THE MIGRATION CAUSES PROBLEMS
-- This will revert everything back to the previous (insecure) state
...
ALTER TABLE public.game_groups DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.leaderboard_cache DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.game_groups_refresh_queue DISABLE ROW LEVEL SECURITY;

-- ============================================================================
-- ROLLBACK: Drop all policies created by the migration
-- ============================================================================
...
DROP POLICY IF EXISTS "Service role can delete achievements" ON public.user_achievements;

-- user_progress policies
DROP POLICY IF EXISTS "Users can view their own progress" ON public.user_progress;
DROP POLICY IF EXISTS "Service role can insert progress" ON public.user_progress;
DROP POLICY IF EXISTS "Service role can update progress" ON public.user_progress;
DROP POLICY IF EXISTS "Service role can delete progress" ON public.user_progress;
...
DROP POLICY IF EXISTS "Anyone can read game groups" ON public.game_groups;
DROP POLICY IF EXISTS "Anyone can read leaderboard cache" ON public.leaderboard_cache;
DROP POLICY IF EXISTS "Service role can manage refresh queue" ON public.game_groups_refresh_queue;

-- ============================================================================
-- ROLLBACK: Restore SECURITY DEFINER on views (old insecure behavior)
-- ============================================================================

-- Note: The original views used SECURITY DEFINER implicitly
-- We'll recreate them with SECURITY DEFINER to match original behavior

-- Restore xbox_leaderboard_cache
...
-- Restore steam_leaderboard_cache
...
-- Restore leaderboard_global_cache
...
-- ROLLBACK COMPLETE
-- ============================================================================
-- Database is now back to the insecure state (RLS disabled)
-- This should only be used temporarily if the migration causes issues
