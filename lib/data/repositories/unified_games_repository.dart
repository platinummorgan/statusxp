...
class UnifiedGamesRepository {
  ...
  Future<List<UnifiedGame>> getUnifiedGames(String userId) async {
    try {
      ...
      return _mapUnifiedGames(response);
    } catch (e) {
      rethrow;
    }
  }

  /// Fetch games for a public target profile (read-only).
  Future<List<UnifiedGame>> getPublicUnifiedGames(String targetUserId) async {
    try {
      final response = await _client.rpc(
        'get_public_user_grouped_games',
  ...
  List<UnifiedGame> _mapUnifiedGames(dynamic response) {
    ...
    for (final group in data) {
      ...
      final coverUrl = kIsWeb
          ? (group['proxied_cover_url'] ?? group['cover_url']) as String?
          : group['cover_url'] as String?;
      final platformsData =
          (group['platforms'] as List?)?.cast<Map<String, dynamic>>() ?? [];
      ...
      for (final platformData in platformsData) {
        ...
        final completion =
            (platformData['completion'] as num?)?.toDouble() ?? 0.0;
        final statusXP = ((platformData['statusxp'] as num?)?.toDouble() ?? 0.0)
            .toInt();
        final gameTitleId = platformData['game_title_id']?.toString() ?? '';
        final currentScore =
            ((platformData['current_score'] as num?)?.toDouble() ?? 0.0)
                .toInt();
        final totalScore =
            ((platformData['total_score'] as num?)?.toDouble() ?? 0.0).toInt();
        ...
        final int earnedCount =
            (platformData['earned_trophies'] as int?) ??
            (platformData['xbox_achievements_earned'] as int?) ??
            0;
        final int totalCount =
            (platformData['total_trophies'] as int?) ??
            (platformData['xbox_total_achievements'] as int?) ??
            0;

        final lastPlayedAtStr = platformData['last_played_at'] as String?;
        final lastTrophyEarnedAtStr =
            platformData['last_trophy_earned_at'] as String?;
        ...
        final rarestRarity = (platformData['rarest_achievement_rarity'] as num?)
            ?.toDouble();
      ...
      final overallCompletion = platforms.isNotEmpty
          ? platforms.map((p) => p.completion).reduce((a, b) => a + b) /
                platforms.length
          : 0.0;
