...
final achievementCheckerServiceProvider = Provider<AchievementCheckerService>((
  ref,
) {
...
final allAchievementsProvider =
    FutureProvider.family<List<MetaAchievement>, String>((ref, userId) async {
      final repository = ref.read(metaAchievementRepositoryProvider);
      // Get user's connected platforms for filtering
      final connectedPlatforms = await ref.watch(
        connectedPlatformsProvider.future,
      );
...
class _AchievementsScreenState extends ConsumerState<AchievementsScreen> {
                ...
                ...unlockedAchievements.map(
                  (achievement) => Padding(
                    ...
                    child: Text(
                      ...
                      style: const TextStyle(color: CyberpunkTheme.neonPurple),
                    ),
                  ),
                ),
              ],
    ...
    return Container(
      ...
      child: ListTile(
        ...
        leading: Container(
          ...
          height: 56,
          decoration: BoxDecoration(
            color: categoryColor.withValues(alpha: isUnlocked ? 0.2 : 0.1),
            borderRadius: BorderRadius.circular(8),
            border: Border.all(
              color: categoryColor.withValues(alpha: isUnlocked ? 0.5 : 0.2),
              width: 2,
            ),
          ),
          child: Center(
        ...
        trailing: !isUnlocked
            ? null
            : Icon(Icons.star, color: categoryColor, size: 28),
      ),
    );
  }
  ...
  Widget _buildPlatformFilters(
    ...
    List<MetaAchievement> achievements,
  ) {
