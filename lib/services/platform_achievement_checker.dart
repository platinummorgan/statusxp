...
class PlatformAchievementChecker {
    ...
    try {
      ...
      if ((stats['psn_total'] ?? 0) > 0) {
        newlyUnlocked.addAll(
          await _checkPSNAchievements(userId, stats, unlocked),
        );
      }
      ...
      if ((stats['xbox_total'] ?? 0) > 0) {
        newlyUnlocked.addAll(
          await _checkXboxAchievements(userId, stats, unlocked),
        );
      }
      ...
      if ((stats['steam_total'] ?? 0) > 0) {
        newlyUnlocked.addAll(
          await _checkSteamAchievements(userId, stats, unlocked),
        );
      }

      // Check cross-platform achievements (requires all 3)
      if ((stats['psn_total'] ?? 0) > 0 &&
          (stats['xbox_total'] ?? 0) > 0 &&
          (stats['steam_total'] ?? 0) > 0) {
        newlyUnlocked.addAll(
          await _checkCrossPlatformAchievements(userId, stats, unlocked),
        );
      }
  ...
  Future<Map<String, int>> _getUserStats(String userId) async {
    ...
    final achievementData = await _client
        ...
        .select(
          'platform_id, achievements(rarity_global, score_value, metadata)',
        )
        .eq('user_id', userId);
    ...
    final completionData = await _client.rpc(
  ...
  Future<List<String>> _checkPSNAchievements(
    ...
    final trophyMilestones = {
      ...
      'psn_10_trophies': 10,
      'psn_50_trophies': 50,
      'psn_100_trophies': 100,
      'psn_250_trophies': 250,
      'psn_500_trophies': 500,
      'psn_1000_trophies': 1000,
      'psn_2500_trophies': 2500,
      'psn_5000_trophies': 5000,
      'psn_7500_trophies': 7500,
      'psn_10000_trophies': 10000,
      'psn_15000_trophies': 15000,
    };
    ...
    final bronzeMilestones = {
      ...
      'psn_100_bronze': 100,
      'psn_500_bronze': 500,
      'psn_1000_bronze': 1000,
      'psn_2500_bronze': 2500,
      'psn_5000_bronze': 5000,
      'psn_7500_bronze': 7500,
      'psn_10000_bronze': 10000,
    };

    for (final entry in bronzeMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['psn_bronze']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
    ...
    final silverMilestones = {
      ...
      'psn_100_silver': 100,
      'psn_500_silver': 500,
      'psn_1000_silver': 1000,
      'psn_2000_silver': 2000,
      'psn_3000_silver': 3000,
    };

    for (final entry in silverMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['psn_silver']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
    ...
    final platinumMilestones = {
      ...
      'psn_25_platinum': 25,
      'psn_50_platinum': 50,
      'psn_100_platinum': 100,
      'psn_150_platinum': 150,
      'psn_200_platinum': 200,
      'psn_250_platinum': 250,
    };

    for (final entry in platinumMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['psn_platinum']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
  ...
  Future<List<String>> _checkXboxAchievements(
    ...
    final achievementMilestones = {
      'xbox_first_unlock': 1,
      'xbox_10_achievements': 10,
      'xbox_50_achievements': 50,
      'xbox_100_achievements': 100,
      'xbox_250_achievements': 250,
      'xbox_500_achievements': 500,
      'xbox_1000_achievements': 1000,
      'xbox_2500_achievements': 2500,
      'xbox_5000_achievements': 5000,
      'xbox_7500_achievements': 7500,
      'xbox_10000_achievements': 10000,
      'xbox_15000_achievements': 15000,
    };

    for (final entry in achievementMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['xbox_total']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
    ...
    final gamerscoreMilestones = {
      ...
      'xbox_50000_gs': 50000,
      'xbox_75000_gs': 75000,
      'xbox_100000_gs': 100000,
      'xbox_150000_gs': 150000,
      'xbox_200000_gs': 200000,
      'xbox_250000_gs': 250000,
      'xbox_300000_gs': 300000,
    };

    for (final entry in gamerscoreMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['xbox_gamerscore']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
    ...
    final completionMilestones = {
      ...
      'xbox_25_complete': 25,
      'xbox_50_complete': 50,
      'xbox_100_complete': 100,
      'xbox_150_complete': 150,
    };

    for (final entry in completionMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['xbox_complete']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
  ...
  Future<List<String>> _checkSteamAchievements(
    ...
    // Achievement total milestones
    final achievementMilestones = {
      'steam_first_unlock': 1,
      'steam_10_achievements': 10,
      'steam_50_achievements': 50,
      'steam_100_achievements': 100,
      'steam_250_achievements': 250,
      'steam_500_achievements': 500,
      'steam_1000_achievements': 1000,
      'steam_2500_achievements': 2500,
      'steam_5000_achievements': 5000,
      'steam_7500_achievements': 7500,
      'steam_10000_achievements': 10000,
      'steam_15000_achievements': 15000,
    };

    for (final entry in achievementMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['steam_total']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
    ...
    final perfectMilestones = {
      ...
      'steam_25_perfect': 25,
      'steam_50_perfect': 50,
      'steam_100_perfect': 100,
      'steam_150_perfect': 150,
    };

    for (final entry in perfectMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['steam_perfect']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
  ...
  Future<List<String>> _checkCrossPlatformAchievements(
    ...
    final statusxpMilestones = {
      'cross_statusxp_500': 500,
      'cross_statusxp_1500': 1500,
      'cross_statusxp_3500': 3500,
      'cross_statusxp_7500': 7500,
      'cross_statusxp_15000': 15000,
      'cross_statusxp_20000': 20000,
    ...
    if (!unlocked.contains('cross_triple_threat') &&
        stats['psn_total']! >= 100 &&
        stats['xbox_total']! >= 100 &&
        stats['steam_total']! >= 100) {
    ...
    if (!unlocked.contains('cross_universal_gamer') &&
        stats['psn_total']! >= 500 &&
        stats['xbox_total']! >= 500 &&
        stats['steam_total']! >= 500) {
    ...
    if (!unlocked.contains('cross_platform_master') &&
        stats['psn_total']! >= 1000 &&
        stats['xbox_total']! >= 1000 &&
        stats['steam_total']! >= 1000) {
    ...
    if (!unlocked.contains('cross_ecosystem_legend') &&
        stats['psn_total']! >= 2500 &&
        stats['xbox_total']! >= 2500 &&
        stats['steam_total']! >= 2500) {
    ...
    final combinedMilestones = {
      'cross_1000_unlocks': 1000,
      'cross_2500_unlocks': 2500,
      'cross_5000_unlocks': 5000,
      'cross_10000_unlocks': 10000,
      'cross_15000_unlocks': 15000,
    };

    for (final entry in combinedMilestones.entries) {
      if (!unlocked.contains(entry.key) &&
          stats['total_unlocks']! >= entry.value) {
        await _unlockAchievement(userId, entry.key);
        newlyUnlocked.add(entry.key);
      }
    }
