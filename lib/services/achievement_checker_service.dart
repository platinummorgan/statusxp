...
class AchievementCheckerService {
  ...
  Future<List<String>> checkAndUnlockAchievements(String userId) async {
    ...
    try {
      ...
      await _checkCrossPlatformAchievements(
        userId,
        crossStats,
        unlockedIds,
        newlyUnlocked,
  ...
  Future<Map<String, dynamic>> _getPSNStats(String userId) async {
    try {
      ...
      final trophyList = (achievements as List).where((row) {
      ...
      final stats = {
        ...
        'bronze': trophyList
            .where(
              (t) =>
                  (t['achievements']?['metadata']?['psn_trophy_type']
                      as String?) ==
                  'bronze',
            )
            .length,
  ...
  Future<Map<String, dynamic>> _getXboxStats(String userId) async {
    try {
      ...
      final achievementList = (achievements as List).where((a) {
  ...
  Future<Map<String, dynamic>> _getCrossStats(String userId) async {
    try {
      ...
      final steamCount = rows
          .where((r) => (r['platform_id'] as int?) == 4)
          .length;
  ...
  /// Check PSN achievements
  Future<void> _checkPSNAchievements(
    String userId,
  ...
  /// Check Xbox achievements
  Future<void> _checkXboxAchievements(
    String userId,
    Map<String, dynamic> stats,
    Set<String> unlocked,
    List<String> newlyUnlocked,
  ...
  /// Check Steam achievements
  Future<void> _checkSteamAchievements(
    String userId,
    Map<String, dynamic> stats,
    Set<String> unlocked,
    List<String> newlyUnlocked,
  ...
  /// Check cross-platform achievements
  Future<void> _checkCrossPlatformAchievements(
    String userId,
    Map<String, dynamic> stats,
    Set<String> unlocked,
    List<String> newlyUnlocked,
    ...
    if (stats['active_platforms'] >= 3) {
      ...
      if (stats['psn_count'] >= 100 &&
          stats['xbox_count'] >= 100 &&
          stats['steam_count'] >= 100) {
        await _check(
          userId,
          'cross_triple_threat',
          1,
      ...
      if (stats['psn_count'] >= 500 &&
          stats['xbox_count'] >= 500 &&
          stats['steam_count'] >= 500) {
        await _check(
          userId,
      ...
      if (stats['psn_count'] >= 1000 &&
          stats['xbox_count'] >= 1000 &&
          stats['steam_count'] >= 1000) {
        await _check(
          userId,
      ...
      if (stats['psn_count'] >= 2500 &&
          stats['xbox_count'] >= 2500 &&
          stats['steam_count'] >= 2500) {
        await _check(
          userId,
