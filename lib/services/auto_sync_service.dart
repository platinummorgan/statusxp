...
class AutoSyncService {
  ...
  Future<AutoSyncResult> _performSync() async {
    ...
    try {
      ...
      if (psnLinked && await _shouldSync(_psnLastSyncKey)) {
        ...
        debugPrint(
          '⏭️ Skipping PSN sync (linked: $psnLinked, should sync: ${await _shouldSync(_psnLastSyncKey)})',
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (xboxLinked && await _shouldSync(_xboxLastSyncKey)) {
        ...
        debugPrint(
          '⏭️ Skipping Xbox sync (linked: $xboxLinked, should sync: ${await _shouldSync(_xboxLastSyncKey)})',
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (steamLinked && await _shouldSync(_steamLastSyncKey)) {
        ...
        debugPrint(
          '⏭️ Skipping Steam sync (linked: $steamLinked, should sync: ${await _shouldSync(_steamLastSyncKey)})',
        );
      }
    } catch (e) {
  ...
  Future<bool> _shouldSync(String prefKey) async {
    try {
      ...
      final platform = prefKey.contains('psn')
          ? 'psn'
          : prefKey.contains('xbox')
          ? 'xbox'
          : 'steam';
      final lastField = platform == 'psn'
          ? 'last_psn_sync_at'
          : platform == 'xbox'
          ? 'last_xbox_sync_at'
          : 'last_steam_sync_at';
      final statusField = platform == 'psn'
          ? 'psn_sync_status'
          : platform == 'xbox'
          ? 'xbox_sync_status'
          : 'steam_sync_status';
      ...
      debugPrint(
        '⏰ Time since last $platform sync: ${timeSinceSync.inHours}h ${timeSinceSync.inMinutes % 60}m',
      );
