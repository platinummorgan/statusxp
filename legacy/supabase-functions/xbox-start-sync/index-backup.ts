...
serve(async (req) => {
  ...
  try {
    ...
    if (existingLog && profile.xbox_sync_status === 'pending') {
      ...
      // If there's an old pending log but profile isn't 'pending', mark it as failed and start fresh
      if (existingLog) {
        console.log(`Found stale pending sync log ${existingLog.id} - marking as failed and starting fresh`);
        await supabase
          .from('xbox_sync_logs')
          .update({ 
            status: 'failed', 
  ...
  try {
    ...
    try {
      ...
      if (hasMoreGames) {
        ...
        console.log('All games synced! Setting status to COMPLETED');
        await supabase
          .from('profiles')
          .update({
            xbox_sync_status: 'completed',
