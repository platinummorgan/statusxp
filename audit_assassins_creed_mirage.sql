-- Audit all entries of Assassin's Creed Mirage across all platforms

-- Get game titles matching Assassin's Creed Mirage
SELECT 
...
WHERE gt.name ILIKE '%Assassin%Creed%Mirage%'
...
SELECT 
    ...
    MAX(gt.updated_at) as last_updated
FROM game_titles gt
...
-- Get all achievement details for Assassin's Creed Mirage
SELECT 
    gt.id as game_title_id,
    gt.name as game_name,
    a.platform,
